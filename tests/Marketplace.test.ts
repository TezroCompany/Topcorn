...
describe('Marketplace', function () {
    ...
    before(async function () {
        ...
        this.bean = await ethers.getContractAt('MockToken', contracts.topcorn);
        ...
        await this.bean.mint(userAddress, toTopcorn("1000000000"))
        ...
        await this.bean.connect(user).approve(this.silo.address, "100000" + "000000000000000000")
        await this.bean.connect(user2).approve(this.silo.address, "100000" + "000000000000000000")
    })


    const getHashFromListing = function (l:any) {
        return ethers.utils.solidityKeccak256(['uint256', 'uint256', 'uint24', 'uint256', 'bool'], l);
      }


      const getHash = async function (tx: any) {
        let receipt = await tx.wait();
        const args = (receipt.events?.filter((x:any) => { return x.event == "PodListingCreated" }))[0].args;
      ...
      const getOrderId = async function (tx:any) {
        let receipt = await tx.wait();
        let idx = (receipt.events?.filter((x:any) => { return x.event == "PodOrderCreated" }))[0].args.orderId;
        return idx;
      }
    ...
    beforeEach(async function () {    
      ...
      await this.field.incrementTotalSoilEE('100000');
      await this.field.connect(user).sowTopcornsAndIndex('1000');
      await this.field.connect(user2).sowTopcornsAndIndex('1000');
    ...
    describe("Pod Listings", async function () {
        ...
        describe("Fill", async function () {
          ...
          describe('revert', async function () {
            ...
            it('Fails to fill Listing, not enough ETH used', async function () {
              ...
              await expect(this.marketplace.connect(user2).buyTopcornsAndFillPodListing(this.listing, 0, 100, { value: '24' })).to.be.revertedWith('PancakeV2Router: EXCESSIVE_INPUT_AMOUNT');
            })
          ...
          describe("Fill listing", async function () {
            ...
            it('Transfer Beans properly', async function () {
              ...
              expect(await this.claim.wrappedTopcorns(userAddress)).to.equal(0);
            })
          ...
          describe("Fill partial listing to wallet", async function () {
            ...
            it('Transfer Beans properly', async function () {
              ...
              expect(await this.claim.wrappedTopcorns(userAddress)).to.equal(this.amountBeansBuyingWith);
            })
          ...
          describe("Fill Beans to wallet", async function () {
            beforeEach(async function () {
              ...
              this.result = await this.marketplace.connect(user2).buyTopcornsAndFillPodListing(this.listing, this.amountTransferringBeans, this.amoutBuyingBeans, { value: 112 })
            ...
            it('Transfer Beans properly', async function () {
              ...
              expect(this.userBeanBalanceAfter.sub(this.userBeanBalance)).to.equal(0);
              expect(await this.claim.wrappedTopcorns(userAddress)).to.equal(this.amoutBuyingBeans);
            })
          ...
          describe("Fill Beans with amount to wallet", async function () {
            ...
            it('Transfer Beans properly', async function () {
              ...
              expect(this.userBeanBalanceAfter.sub(this.userBeanBalance)).to.equal(0);
              expect(await this.claim.wrappedTopcorns(userAddress)).to.equal(this.amoutBuyingBeans + this.amountTransferringBeans);
            })
          ...
          describe("Fill Beans with amount to wallet and Eth Refund", async function () {
            beforeEach(async function () {
              ...
              this.result = await this.marketplace.connect(user2).buyTopcornsAndFillPodListing(this.listing, this.amountTransferringBeans, this.amoutBuyingBeans, { value: 115 })
          ...
          describe("Claim", async function () {
            beforeEach(async function () {
              await this.silo.connect(user2).depositTopcorns('250')
              await this.silo.connect(user2).withdrawTopcorns([2], ['250'])
            ...
            describe("Claim and fill listing", async function () {
              ...
              it('Transfer Beans properly', async function () {
                expect(this.user2BeanBalance.sub(this.user2BeanBalanceAfter)).to.equal(0);
                expect(await this.claim.wrappedTopcorns(user2Address)).to.equal('150');
            ...
            describe("Claim and fill listing overallocate", async function () {
              ...
              it('Transfer Beans properly', async function () {
                ...
                expect(await this.claim.wrappedTopcorns(user2Address)).to.equal('0');
            ...
            describe("Claim and buy listing to wrapped", async function () {
              ...
              it('Transfer Beans properly', async function () {
                ...
                expect(await this.claim.wrappedTopcorns(user2Address)).to.equal('150');
                expect(this.userBeanBalanceAfter.sub(this.userBeanBalance)).to.equal(0);
                expect(await this.claim.wrappedTopcorns(userAddress)).to.equal('100');
              })
            })
            ...
            describe("Claim, fill Beans with amount to wallet", async function () {
              beforeEach(async function () {
                ...
                this.result = await this.marketplace.connect(user2).claimBuyTopcornsAndFillPodListing(this.listing, this.amountTransferringBeans, this.amoutBuyingBeans, [['27'], [], [], false, false, 0, 0, false], { value: 112 })
        ...
        describe("Cancel", async function () {
          it('Re-list plot cancels and re-lists', async function () {
            let result = await this.marketplace.connect(user).createPodListing('0', '0', '1000', '500000', '0', true);
          ...
          it('Cancels Listing, Emits Listing Cancelled Event', async function () {
            let result = await this.marketplace.connect(user).createPodListing('0', '0', '1000', '500000', '2000', true);
            expect(await this.marketplace.podListing(0)).to.be.equal(await getHash(result));
      ...
      describe("Pod Offers", async function () {
        describe("Create", async function () {
          ...
          describe('buy beans and create order', async function () {
            beforeEach(async function () {
              ...
              this.userBeanBalance = await this.bean.balanceOf(userAddress)
              this.beanstalkBeanBalance = await this.bean.balanceOf(this.marketplace.address)
              this.result = await this.marketplace.connect(user).buyTopcornsAndCreatePodOrder('0', '250', '100000', '1000', { value: 112 })
              this.id = await getOrderId(this.result)
              this.orderIds.push(this.id)
              this.userBeanBalanceAfter = await this.bean.balanceOf(userAddress)
              this.beanstalkBeanBalanceAfter = await this.bean.balanceOf(this.marketplace.address)
          ...
          describe('buy and transfer beans and create order', async function () {
            beforeEach(async function () {
              ...
              this.userBeanBalance = await this.bean.balanceOf(userAddress)
              this.beanstalkBeanBalance = await this.bean.balanceOf(this.marketplace.address)
              this.result = await this.marketplace.connect(user).buyTopcornsAndCreatePodOrder('100', '250', 100000, '1000', { value: 112 })
              this.id = await getOrderId(this.result)
              this.orderIds.push(this.id)
              this.userBeanBalanceAfter = await this.bean.balanceOf(userAddress)
              this.beanstalkBeanBalanceAfter = await this.bean.balanceOf(this.marketplace.address)
          ...
          describe("Claim", async function () {
            beforeEach(async function () {
              await this.silo.connect(user).depositTopcorns('250')
              await this.silo.connect(user).withdrawTopcorns([2], ['250'])
              await this.season.farmSunrises('25')
            })
            ...
            describe('normal under overallocate', async function () {
              ...
              it('Transfer Beans properly', async function () {
                expect(this.beanstalkBeanBalanceAfter.sub(this.beanstalkBeanBalance)).to.equal('0');
                expect(this.userBeanBalance.sub(this.userBeanBalanceAfter)).to.equal('0');
                expect(await this.claim.wrappedTopcorns(userAddress)).to.equal('150');
              })
            ...
            describe('buy and claim', async function () {
              beforeEach(async function () {
                ...
                this.userBeanBalance = await this.bean.balanceOf(userAddress)
                this.beanstalkBeanBalance = await this.bean.balanceOf(this.marketplace.address)
                this.result = await this.marketplace.connect(user).claimBuyTopcornsAndCreatePodOrder('100', '250', '100000', '1000', [['27'], [], [], false, false, 0, 0, false], { value: 112 })
                this.id = await getOrderId(this.result)
                this.orderIds.push(this.id)
                this.userBeanBalanceAfter = await this.bean.balanceOf(userAddress)
                this.beanstalkBeanBalanceAfter = await this.bean.balanceOf(this.marketplace.address)
        ...
        describe("Fill", async function () {
          ...
          describe("revert", async function () {
            ...
            it("plot amount too large", async function () {
              await this.field.connect(user2).sowTopcornsAndIndex('1200');
            ...
            it("sell too much", async function () {
              await expect(this.marketplace.connect(user2).fillPodOrder(this.order, 1000, 0, 1000, false)).to.reverted;
              //await expect(this.marketplace.connect(user2).fillPodOrder(this.order, 1000, 0, 1000, false)).to.be.revertedWithPanic(0x11);
          ...
          describe("Full order to wallet", async function () {
            ...
            it('Transfer Beans properly', async function () {
              ...
              expect(await this.claim.wrappedTopcorns(user2Address)).to.equal('50');
            })
        ...
        describe("Cancel", async function () {
          ...
          describe('Cancel to wrapped', async function () {
            ...
            it('transfer beans', async function () {
              ...
              expect(this.userBeanBalanceAfter.sub(this.userBeanBalance)).to.equal('0');
              expect(await this.claim.wrappedTopcorns(userAddress)).to.equal('500');
            })
